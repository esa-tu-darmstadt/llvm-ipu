...
static DecodeStatus DecodeARRegisterClass(llvm::MCInst &Inst, unsigned RegNo,
                                          uint64_t Address,
                                          const void *Decoder);

static DecodeStatus DecodeMRRegisterClass(llvm::MCInst &Inst, unsigned RegNo,
                                          uint64_t Address,
                                          const void *Decoder);
...
static DecodeStatus DecodeMEMrrOperand(MCInst &Inst, uint64_t Imm,
                                       int64_t Address, const void *Decoder);

static DecodeStatus DecodeMEMrrrOperand(MCInst &Inst, uint64_t Imm,
                                        int64_t Address, const void *Decoder);

static DecodeStatus DecodeMEMrriOperand(MCInst &Inst, uint64_t Imm,
                                        int64_t Address, const void *Decoder);
...
template <class T>
static bool readInstruction(ArrayRef<uint8_t> Bytes, uint64_t Address,
                            uint64_t &Size, T &Insn) {
  if (Bytes.size() < sizeof(T)) {
    Size = 0;
    return false;
  }

  Insn = llvm::support::endian::read<T, llvm::endianness::little,
...
static DecodeStatus DecodeMRRegisterClass(MCInst &Inst, unsigned RegNo,
                                          uint64_t Address,
                                          const void *Decoder) {
...
static DecodeStatus DecodeMRPairRegisterClass(MCInst &Inst, unsigned RegNo,
                                              uint64_t Address,
                                              const void *Decoder) {
  static const uint16_t RegPairDecoderTable[] = {Colossus::MD0, Colossus::MD1,
                                                 Colossus::MD2, Colossus::MD3,
                                                 Colossus::MD4, Colossus::MD5};
...
static DecodeStatus DecodeARRegisterClass(MCInst &Inst, unsigned RegNo,
                                          uint64_t Address,
                                          const void *Decoder) {
  auto Dis = static_cast<const ColossusDisassembler *>(Decoder);
  const MCRegisterInfo *RegInfo = Dis->getContext().getRegisterInfo();
...
static DecodeStatus DecodeARPairRegisterClass(MCInst &Inst, unsigned RegNo,
                                              uint64_t Address,
                                              const void *Decoder) {
...
static DecodeStatus DecodeARQuadRegisterClass(MCInst &Inst, unsigned RegNo,
                                              uint64_t Address,
                                              const void *Decoder) {
...
MCDisassembler::DecodeStatus ColossusDisassembler::getSingleInstruction(
  ...
  if (Result == Fail) {
    ...
    Result = decodeInstruction(DecoderTable32, Instr, Insn32, Address, this,
                               STICopy);
  }
